...
import ghubtty.commentlink
import ghubtty.palette
import ghubtty.keymap
...
DEFAULT_CONFIG_PATH = '~/.config/ghubtty/ghubtty.yaml'
FALLBACK_CONFIG_PATH = '~/.ghubtty.yaml'
...
class Config(object):
    def __init__(self, server=None, palette='default', keymap='default',
        ...
        self.dburi = server.get('dburi',
                                'sqlite:///' + os.path.expanduser('~/.ghubtty.db'))
        socket_path = server.get('socket', '~/.ghubtty.sock')
        self.socket_path = os.path.expanduser(socket_path)
        log_file = server.get('log-file', '~/.ghubtty.log')
        self.log_file = os.path.expanduser(log_file)
        lock_file = server.get('lock-file', '~/.ghubtty.%s.lock' % server['name'])
        self.lock_file = os.path.expanduser(lock_file)

        self.palettes = {'default': ghubtty.palette.Palette({}),
                         'light': ghubtty.palette.Palette(ghubtty.palette.LIGHT_PALETTE),
                         }
        for p in self.config.get('palettes', []):
            if p['name'] not in self.palettes:
                self.palettes[p['name']] = ghubtty.palette.Palette(p)
            else:
                self.palettes[p['name']].update(p)
        self.palette = self.palettes[self.config.get('palette', palette)]

        self.keymaps = {'default': ghubtty.keymap.KeyMap({}),
                        'vi': ghubtty.keymap.KeyMap(ghubtty.keymap.VI_KEYMAP)}
        for p in self.config.get('keymaps', []):
            if p['name'] not in self.keymaps:
                self.keymaps[p['name']] = ghubtty.keymap.KeyMap(p)
            else:
                self.keymaps[p['name']].update(p)
        self.keymap = self.keymaps[self.config.get('keymap', keymap)]

        self.commentlinks = [ghubtty.commentlink.CommentLink(c)
                             for c in self.config.get('commentlinks', [])]
        self.commentlinks.append(
            ghubtty.commentlink.CommentLink(dict(
                    match="(?P<url>https?://\\S*)",
                    replacements=[
                        dict(link=dict(
                                text="{url}",
