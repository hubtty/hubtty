...
def upgrade():
    ...
    for (pname, cnumber, cstatus, rkey, rnumber, commit, parent) in qres.fetchall():
        ...
        repo = ghubtty.gitrepo.get_repo(pname, context.config.ghubtty_app.config)
        try:
        ...
        for stat in stats:
            ...
            m = pathre.match(path)
            status = ghubtty.db.File.STATUS_MODIFIED
            old_path = None
            if m:
                status = ghubtty.db.File.STATUS_RENAMED
                pre = m.group(2) or ''
                post = m.group(6) or ''
