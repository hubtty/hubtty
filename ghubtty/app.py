...
from ghubtty import db
from ghubtty import config
from ghubtty import gitrepo
from ghubtty import keymap
from ghubtty import mywid
from ghubtty import palette
from ghubtty import sync
from ghubtty import search
from ghubtty import requestsexceptions
from ghubtty.view import change_list as view_change_list
from ghubtty.view import project_list as view_project_list
from ghubtty.view import change as view_change
import ghubtty.view
import ghubtty.version
...
interested in, and then press "s" to subscribe to them.  Ghubtty will
...
class App(object):
    ...
    def __init__(self, server=None, palette='default',
        ...
        self.log = logging.getLogger('ghubtty.App')
        ...
        try:
            ...
            print("error: another instance of ghubtty is running for: %s" % self.config.server['name'])
            sys.exit(1)
    ...
    def doSearch(self, query):
        ...
        with self.db.getSession() as session:
            ...
            except ghubtty.search.SearchSyntaxError as e:
        ...
        except ghubtty.view.DisplayError as e:
            return self.error(e.message)
...
def version():
    return "Ghubtty version: %s" % ghubtty.version.version_info.release_string()
...
def main():
    ...
    parser.add_argument('--open', nargs=1, action=OpenChangeAction,
                        metavar='URL',
                        help='open the given URL in a running Ghubtty')
    parser.add_argument('--version', dest='version', action='version',
                        version=version(),
                        help='show Ghubtty\'s version')
