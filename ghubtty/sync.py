...
from ghubtty.auth import FormAuth
...
class Task(object):
    def __init__(self, priority=NORMAL_PRIORITY):
        self.log = logging.getLogger('ghubtty.sync')
...
class SyncChangeTask(Task):
    ...
    def _syncChange(self, sync):
        ...
        for remote_commit, remote_revision in remote_change.get('revisions', {}).items():
            ...
            remote_revision['_ghubtty_remote_comments_data'] = remote_comments_data
            remote_robot_comments_data = sync.get('changes/%s/revisions/%s/robotcomments' % (
                self.change_id, remote_commit))
            remote_revision['_ghubtty_remote_robot_comments_data'] = remote_robot_comments_data
            ...
            if sync.checks_plugin:
                ...
                if remote_checks_data is None:
                    ...
                    remote_revision['_ghubtty_remote_checks_data'] = remote_checks_data
        try:
        ...
        with app.db.getSession() as session:
            ...
            for remote_commit, remote_revision in remote_change.get('revisions', {}).items():
                ...
                remote_comments_items = list(remote_revision['_ghubtty_remote_comments_data'].items())
                remote_robot_comments_items = list(remote_revision.get('_ghubtty_remote_robot_comments_data', {}).items())
                ...
                if remote_revision.get('_ghubtty_remote_checks_data'):
                    self._updateChecks(session, change, revision, remote_revision['_ghubtty_remote_checks_data'])
            # End revisions
            new_message = False
...
class Sync(object):
    def __init__(self, app, disable_background_sync):
        self.user_agent = 'Ghubtty/%s %s' % (ghubtty.version.version_info.release_string(),
