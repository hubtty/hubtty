...
class SearchCompiler(object):
    ...
    def parse(self, data):
        ...
        if gertty.db.file_table in tables:
            ...
            # revision.
            s = select([func.max(gertty.db.revision_table.c.number)], correlate=False).where(
                gertty.db.revision_table.c.change_key==gertty.db.change_table.c.key).correlate(gertty.db.change_table)
            result = and_(gertty.db.file_table.c.revision_key == gertty.db.revision_table.c.key,
                          ...
                          gertty.db.revision_table.c.number == s,
                          result)
